...
はじめから:
  ...
  * `--max-selections=N`: 1回の質問での選択肢の数の最大値を `N` に設定します。
...
#### コマンドリスト
  ...
  * `draw-tree [--from=[root]|--from-selection=[sel]] --depth=[depth] --output=[name]`

    端末に現在のヒープ木を描画します。

    `[root]`: 表示開始位置を指定します。全て表示するには 0 を指定します。
    
    `[sel]`: 表示開始位置を現在の選択肢の番号で指定します。

    `[depth]`: 表示する深さの限界を指定します。省略したり、0 を指定すると無制限に表示します。
    
    `[name]`: ファイルに書き出したい場合、その名前。

  * `draw-dot [--from=[root]|--from-selection=[sel]] --depth=[depth] --output=[name]`

    現在のヒープ木 (全体) を dot 言語で出力する以外は `draw-tree` と同じです。
